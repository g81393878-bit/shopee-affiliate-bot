...
from app.api.facebook_radar import require_admin_auth
...
# ---------------------------------------------------------------------------
# คิวแชร์โพสต์เพจลงกลุ่ม — โพสต์เพจสำเร็จเข้าคิว → บอท local poll → แชร์ → รายงาน
# ---------------------------------------------------------------------------
GROUP_SHARE_POLL_BATCH_SIZE = 20               # จำกัดงานต่อรอบ poll (กัน FB จับรัว ๆ)
GROUP_SHARE_CLAIM_TIMEOUT_MINUTES = 30         # claim ค้างเกินนี้ = บอทตาย → ปล่อยคืนคิว


@router.get("/api/admin/group-shares/pending")
def get_pending_group_shares(_: None = Depends(require_admin_auth)):
    """บอทแชร์ local poll งานแชร์โพสต์เพจที่ค้างอยู่ — claim ก่อนคืน (กัน poll ซ้ำ)

    self-heal: งานที่ claim ค้างเกิน GROUP_SHARE_CLAIM_TIMEOUT_MINUTES (บอทตายกลางทาง)
    → ปล่อยกลับเป็น pending ให้ poll ใหม่
    """
    now = datetime.datetime.now(datetime.timezone.utc)
    db = _db()
    try:
        stale_cutoff = now - datetime.timedelta(minutes=GROUP_SHARE_CLAIM_TIMEOUT_MINUTES)
        stale = (db.query(models.GroupShareTask)
                  .filter(models.GroupShareTask.status == "claimed",
                          models.GroupShareTask.claimed_at < stale_cutoff).all())
        for t in stale:
            t.status = "pending"
            t.claimed_at = None
        if stale:
            db.commit()
        tasks = (db.query(models.GroupShareTask)
                  .filter(models.GroupShareTask.status == "pending")
                  .order_by(models.GroupShareTask.id)
                  .limit(GROUP_SHARE_POLL_BATCH_SIZE).all())
        for t in tasks:
            t.status = "claimed"
            t.claimed_at = now
        if tasks:
            db.commit()
        return [{"task_id": t.id, "post_url": t.post_url, "kind": t.kind,
                 "post_id": t.post_id} for t in tasks]
    finally:
        db.close()


@router.post("/api/admin/group-shares/{task_id}/status")
def report_group_share_status(task_id: int, status: str = Query(...),
                              note: str = "", _: None = Depends(require_admin_auth)):
    """บอท local รายงานผลแชร์: status = shared | failed | skipped"""
    if status not in ("shared", "failed", "skipped"):
        raise HTTPException(status_code=400, detail="status ต้องเป็น shared/failed/skipped")
    db = _db()
    try:
        t = db.query(models.GroupShareTask).filter_by(id=task_id).first()
        if not t:
            raise HTTPException(status_code=404, detail="ไม่พบงานแชร์นี้")
        t.status = status
        t.note = (note or "")[:500]
        if status == "shared":
            t.shared_at = datetime.datetime.now(datetime.timezone.utc)
        db.commit()
...
@router.get("/api/admin/group-shares")
def list_group_shares(status: str = "", kind: str = "",
                      limit: int = Query(100, ge=1, le=500),
                      _: None = Depends(require_admin)):
    """รายการคิวแชร์ทั้งหมด (หน้าแอดมิน) — filter สถานะ/ชนิด"""
    db = _db()
    try:
        q = db.query(models.GroupShareTask)
        if status:
            q = q.filter(models.GroupShareTask.status == status)
        if kind:
            q = q.filter(models.GroupShareTask.kind == kind)
        rows = q.order_by(models.GroupShareTask.id.desc()).limit(limit).all()
        return {"total": len(rows), "items": [{
            "id": t.id, "post_url": t.post_url, "kind": t.kind,
            "post_id": t.post_id, "status": t.status, "note": t.note,
            "created_at": (t.created_at or datetime.datetime.now(datetime.timezone.utc)).isoformat(),
            "shared_at": t.shared_at.isoformat() if t.shared_at else None,
        } for t in rows]}
    finally:
        db.close()
