...
LINE_OA_URL = os.getenv("LINE_OA_URL") or ""


def _build_intro(line_oa_url: str = "") -> str:
    """ข้อความแนะนำบอทป้าเข็ม — ถ้ามีลิงก์ LINE OA แปะลิงก์ให้กดแอดไลน์ได้ทันที"""
    cta = (
        f"👉 แอดไลน์ป้าเข็มได้เลย: {line_oa_url}"
        if line_oa_url
        else "👉 คุยกับป้าเข็ม 24 ชม. ได้ที่ LINE แอดไลน์ร้าน \"ป้าเข็ม ขายของ\" ได้เลยนะคะ"
    )
    return (
...
BOT_INTRO = _build_intro(LINE_OA_URL)
