...
def chat_with_mom_khem(user_text: str, user_name: str) -> Optional[str]:
    """คุยเรื่องบอทผ่าน Groq — ตอบเรื่องฟีเจอร์/บริการ/พูดคุยทั่วไป (แยกเด็ดขาดจาก
    คำถามสินค้า: เรื่องสินค้าต้องผ่าน search_products ข้อมูลจริงเท่านั้น)
    prompt ห้าม Groq ตอบข้อมูลสินค้าเด็ดขาด (กันมโนราคา/ยอดขาย)
    ...
    try:
        ...
        system = (
            ...
            "บทบาท: ตอบคำถามลูกค้าเกี่ยวกับ 'บอท/ร้าน' เท่านั้น — ฟีเจอร์ วิธีใช้ บริการ "
            "และพูดคุยทั่วไป อย่างเป็นกันเอง ใช้ภาษาไทยเหมือนคุณป้าขายของใจดี "
            "พูดสั้น กระชับ (ไม่เกิน ~6 บรรทัด) ไม่โฆษณาเกินจริง\n\n"
            "สิ่งที่บอททำได้ (อธิบายให้ลูกค้าฟังเมื่อถาม):\n"
            ...
            "กฎเด็ดขาด (ห้ามละเมิดเด็ดขาด):\n"
            "- ห้ามตอบข้อมูลสินค้าโดยตรง — ห้ามบอกราคา/ยอดขาย/รีวิว/สต็อก/โปรโมชัน "
            "หรือเทียบสินค้าเด็ดขาด เพราะคุณไม่มีข้อมูลจริง\n"
            "- ถ้าลูกค้าถามเรื่องสินค้า (เช่น \"หูฟังราคาเท่าไหร่\" \"มีกระติกไหม\") "
            "ให้ตอบสั้นๆ ว่า ลองพิมพ์ชื่อสินค้าให้ป้าเข็มหาให้จ๊ะ เช่น 'หูฟังไม่เกิน 300' 😊 "
            "แล้วหยุด — ปล่อยให้บอทค้นสินค้าตอบด้วยข้อมูลจริง\n"
            "- ไม่มโน ไม่เดา ไม่แนะนำยี่ห้อ/ราคาใดๆ ทั้งสิ้น\n"
            "- ถ้าลูกค้ายืนยันอยากคุยกับคนจริง ให้ตอบสุภาพว่าแม่เข็มจะมาตอบกลับที่แชทนี้โดยเร็ว"
        )
        last_err = None
        ...
        if normalized_text in DELETE_PHRASES:
            ...
            if hits:
                ...
                if alt:
                    ...
                    # แยกเด็ดขาด: เรื่องสินค้า → บอทค้นข้อมูลจริง (ข้างบน) / เรื่องบอท → Groq
                    # มาถึงตรงนี้ = ไม่ใช่คำถามสินค้า (ค้นไม่เจอ/ไม่มีใกล้เคียง) → Groq อธิบายฟีเจอร์/บริการ
