...
# มาตรฐาน 5 ขั้นตอน (แบบสั้นสำหรับเมนู/ทักทาย — ข้อย่อยเต็มอยู่ที่ FAQ "บริการ")
FIVE_STEPS_LINES = (
...
FIVE_STEPS_INTRO = f"ป้าเข็มดูแลคุณตามมาตรฐาน 5 ขั้นตอนจ๊ะ:\n{FIVE_STEPS_LINES}"


def greeting_text_for(user_name: str, tone: str = "neutral") -> str:
    """ทักทาย + ทางเลือก ปรับโทนตามวัย (neutral = ข้อความเดิม) — ทุกโทนเปิดด้วยมาตรฐาน 5 ขั้นตอน"""
    if tone == "youth":
        return (
...
หรือใส่เงื่อนไข "หูฟังไม่เกิน 300" ป้าเข็มจัดให้ไวๆ จ้า 😎"""
        )
    if tone == "elder":
        return (
...
เช่น "หูฟัง" หรือ "กระติกน้ำ" หรือกดปุ่มข้างล่างก็ได้นะคะ ไม่ต้องรีบค่ะ"""
        )
...
def welcome_text(user_name: str, tone: str = "neutral") -> str:
    """ข้อความต้อนรับแรก (คุณค่าชัด + มาตรฐาน 5 ขั้นตอน) — ปรับสำเนียงตามวัย (neutral = เดิม)"""
    if tone == "youth":
        return (
...
def greeting_text(user_name: str) -> str:
    """แนวสากล: ทักทายด้วยมาตรฐาน 5 ขั้นตอน + ทางเลือก — ไม่ยิงสินค้าจนกว่าลูกค้าบอกความต้องการ"""
    return (
        f"🤗 สวัสดีค่ะคุณ {user_name}! ยินดีต้อนรับสู่ร้าน{BOT_NAME} 💕\n\n"
        f"{FIVE_STEPS_INTRO}\n\n"
...
BOT_MANUAL_PHRASES = (
    ...
    # แพ็กเกจ/ราคาบอท (ขายต่อ) — ใช้คำเฉพาะ ไม่ใช้ "แพ็กเกจ" เดี่ยว (ชนชื่อสินค้า 3 ตัว)
    ...
    # Lean Stack / Shopee Affiliate / ไฟล์สินค้า / ฟีเจอร์เสริม (ขายบอท — เติมเพิ่ม)
    "ลีนสแต็ค", "leanstack", "บอทง่าย", "บอทถูก", "ตอบkeyword", "keyword", "googlesheets",
    "แอปสคริปต์", "appsscript", "sheets",
    "แอฟฟิลิเอต", "affiliate", "นายหน้า", "แปลงลิงก์", "ทำลิงก์ค่าคอม",
    "เฉพาะshopee", "ใช้เฉพาะ", "ลาซาด้า", "lazada", "ติ๊กต็อก", "แพลตฟอร์มอื่น", "ใช้กับshopee",
    "ไฟล์สินค้า", "ไฟล์csv", "csv", "คอลัมน์", "ลิงก์สินค้า", "ลิงก์ข้อเสนอ",
    "ฟีเจอร์เสริม", "addon", "add-on", "ฟีเจอร์เพิ่ม",
    "ค่าบริการไลน์", "ไลน์แพ็กเกจ",
    # ความน่าเชื่อถือ/กันมิจฉาชีพ (ลูกค้าขี้สงสัย) — คำเฉพาะ 0 ชนชื่อสินค้า
    "เชื่อถือ", "น่าเชื่อ", "ไว้ใจ", "โกง", "มิจฉาชีพ", "หลอกลวง", "เชื่อได้ไหม", "โดนหลอก",
    # ราคา/ไม่บวกเพิ่ม (ลูกค้าขี้สงสัยกลัวโดนบวกราคา) — 0 ชนชื่อสินค้า
    "บวกเพิ่ม", "บวกเงิน", "บวกราคา", "เท่ากับแอป", "ราคาเดียวกัน", "ขึ้นราคา", "คิดเงิน",
    "ของหมด", "ลิงก์ตาย",
    ...
    "ทำไมรายเดือน", "จ่ายรายเดือน", "จ่ายทุกเดือน", "ทำไมต้องจ่าย", "ต้องจ่าย",
    ...
    "โอนมัดจำ", "ชำระค่ามัดจำ", "โอนค่ามัดจำ",
...
PACKAGE_KWS = (
    "ค่าบริการ", "แพ็กเกจราคา", "ราคาแพ็กเกจ", "ราคาบอท", "แพ็กเกจ",
    "สมัครใช้", "สมัครบอท", "สมัครสมาชิก", "สมัครแพ็กเกจ",
    "ซื้อบอท", "ขายบอท", "สนใจบอท", "อยากได้บอท", "เปิดร้าน", "ระบบบอท",
)

# ขายขาด/ซื้อครั้งเดียว (แม่ค้าไม่อยากผูกเดือน) — ทางเลือกซื้อขาดจบครั้งเดียว
PERPETUAL_KWS = (
    "ขายขาด", "ซื้อขาด", "ซื้อครั้งเดียว", "เหมาจ่าย", "จ่ายครั้งเดียว",
)
...
# วิธีจ่ายเงินค่าบอท/มัดจำ (แม่ค้าถามจ่ายยังไง) — คำเฉพาะ 0 ชนชื่อสินค้า
# หลังโฟกัสขายบอท (เจ้าของสั่ง 2026-08): "ชำระเงิน/จ่ายเงิน/โอนเงิน" เฉย ๆ ก็ตอบวิธีจ่ายค่าบอท
# (ดึงคำพวกนี้ออกจาก FAQ "จ่ายที่ Shopee" แล้ว — สั่งซื้อสินค้ายังใช้ "สั่งซื้อ/ซื้อยังไง/วิธีซื้อ")
BOT_PAYMENT_KWS = (
    "จ่ายค่าบอท", "จ่ายมัดจำ", "จ่ายแพ็กเกจ", "วิธีจ่ายค่าบอท",
    "promptpay", "พร้อมเพย์", "บัตรเครดิต", "ตัดบัตร",
    "โอนค่าบอท", "ชำระค่าบอท", "จ่ายค่างวด", "ค่างวด",
...
def _payment_method_lines() -> list:
    """บรรทัดเลขพร้อมเพย์/บัญชีธนาคาร + บัตร — ใช้ทั้งหน้าชำระรวมและหน้าชำระรายแพ็กเกจ"""
    pay_lines = []
    if OWNER_PROMPTPAY:
...
def payment_reply_text() -> str:
    """ข้อความวิธีจ่ายเงินค่าบอท — รายเดือนจ่ายเต็มเดือนแรกก่อนเริ่มทำ (ไม่มีมัดจำ/ค่าติดตั้งแยก) · ขายขาดมัดจำ 50% จ่ายครั้งเดียว"""
    return (
        "💳 วิธีจ่ายเงินค่าบอท จ๊ะ:\n\n"
        "💰 แพ็กเกจรายเดือน — จ่ายเต็มเดือนแรกก่อนเริ่มทำ (ไม่มีมัดจำ ไม่มีค่าติดตั้งแยก):\n"
        "🟡 Lean 490       → จ่าย 490 ก่อนเริ่ม\n"
        "🟢 Starter 990    → จ่าย 990 ก่อนเริ่ม\n"
        "🔵 Business 1,990 → จ่าย 1,990 ก่อนเริ่ม\n"
        "🟣 White-Label 4,990 → จ่าย 4,990 ก่อนเริ่ม\n"
        "   หลังรับบอท จ่ายรายเดือนต่อตามแพ็กเกจที่เลือก\n\n"
        "🟠 ขายขาด 15,000–25,000 → มัดจำ 50% (7,500–12,500) ก่อนเริ่ม\n"
        "   ส่งมอบจ่ายที่เหลือ 7,500–12,500 · จ่ายครั้งเดียวจบ ไม่มีรายเดือน\n\n"
...
# ยอดจ่ายก่อนเริ่ม/หลังรับบอท รายแพ็กเกจ (ปุ่ม Quick Reply แยก 5 แพ็กเกจ — ลูกค้าแตะเห็นยอดตัวนั้นชัด ๆ)
PACKAGE_PAYMENTS = (
    {"key": "lean", "label": "🟡 Lean", "name": "🟡 Lean 490",
     "first": "490 บาท", "monthly": "490 บาท/เดือน"},
    {"key": "starter", "label": "🟢 Starter", "name": "🟢 Starter 990",
     "first": "990 บาท", "monthly": "990 บาท/เดือน"},
    {"key": "business", "label": "🔵 Business", "name": "🔵 Business 1,990",
     "first": "1,990 บาท", "monthly": "1,990 บาท/เดือน"},
    {"key": "whitelabel", "label": "🟣 White-Label", "name": "🟣 White-Label 4,990",
     "first": "4,990 บาท", "monthly": "4,990 บาท/เดือน"},
    {"key": "ขายขาด", "label": "🟠 ขายขาด", "name": "🟠 ขายขาด 15,000–25,000",
     "deposit": "7,500–12,500 บาท", "delivery": "7,500–12,500 บาท",
     "total": "15,000–25,000 บาท"},
)
...
BOT_MANUAL_SECTIONS = [
    ...
    (("ลิขสิทธิ์", "license", "ขายต่อ", "ไวรัส", "อันตราย"),
     "✅ บอทป้าเข็มเป็นสินค้ามีลิขสิทธิ์ของร้าน (ไม่มีไวรัส ปลอดภัย) — ขายเป็นแพ็กเกจ 990/1,990/4,990 "
    ...
    (("แอฟฟิลิเอต", "affiliate", "นายหน้า", "แปลงลิงก์", "ทำลิงก์ค่าคอม", "ลิงก์ค่าคอม"),
     "📦 Shopee Affiliate = โปรแกรมนายหน้าของ Shopee (สมัครฟรี) — ได้ลิงก์ค่าคอมมาใส่ในบอท\n"
     "วิธีทำลิงก์: ผ่านมือถือ (tools/shopee_affiliate.py) หรือ export CSV จาก affiliate.shopee.co.th\n"
     "ระบบตรวจลิงก์ทุกตัวก่อนโชว์ (ลิงก์ตาย/ปลอมไม่ขึ้น) ลูกค้ากดซื้อผ่านลิงก์ → ร้านได้ค่าคอม "
     "(ลูกค้าจ่ายราคาเท่าเดิม ไม่บวกเพิ่ม) จ๊ะ"),
    ...
    (("ราคาเท่าไหร่", "ราคาเท่าไร"),
     "🏷️ การ์ดไม่แสดงราคาตายตัวจ๊ะ — พิมพ์ชื่อที่อยากได้ เช่น \"หูฟัง\" แล้วป้าเข็มหาให้ "
     "จากนั้นแตะดูราคาล่าสุดตามตัวเลือกและโปรโมชันใน Shopee ได้เลย"),
    ...
    (("ราคาเปลี่ยน", "ราคาไม่ตรง", "อัปเดต", "ของหมด", "ลิงก์ตาย", "ของปลอม"),
     ...
     "แต่ไม่ยืนยันราคาตายตัว เพราะขึ้นกับตัวเลือก คูปอง และโปรโมชัน — ให้ยึดราคาล่าสุดใน Shopee จ๊ะ"),
    (("สั่งซื้อ", "สั่งยังไง", "ซื้อยังไง", "ซื้ออย่างไร", "ซื้อสินค้าอย่างไร", "ซื้อสินค้ายังไง", "ซื้อของอย่างไร", "ซื้อของยังไง", "วิธีซื้อ", "วิธีสั่ง", "วิธีสั่งซื้อ"),
     "🛒 สั่งซื้อผ่าน Shopee โดยตรงจ๊ะ — แตะปุ่ม \"ดูราคาล่าสุดใน Shopee\" ในแชท → จ่ายที่ Shopee "
    ...
    (("ค่าส่ง", "ค่าจัดส่ง", "ส่งฟรี", "ขนส่ง"),
     "📦 ค่าส่งและยอดสุดท้ายขึ้นกับร้านค้า ตัวเลือก และคูปอง — ดูยอดล่าสุดใน Shopee ก่อนชำระ ป้าเข็มไม่คิดเพิ่มจ๊ะ"),
    (("คูปอง", "โปรโมชั่น", "โปร", "ส่วนลด", "ลดราคา", "มีโปร", "ของลดราคา"),
     "🏷️ โปรโมชันและคูปองเปลี่ยนได้ตามบัญชีและตัวเลือกสินค้า — แตะการ์ดเพื่อดูราคาล่าสุดใน Shopee "
     "หรือถาม \"มีอะไรใหม่\" ได้เลยจ๊ะ"),
    (("บอทไม่ตอบ", "บอทเงียบ"),
    ...
    # --- Lean Stack / แพลตฟอร์ม / ไฟล์สินค้า / ฟีเจอร์เสริม / ค่า LINE OA (ขายบอท) ---
    (("ลีนสแต็ค", "leanstack", "บอทง่าย", "บอทถูก", "ตอบkeyword", "keyword", "googlesheets", "แอปสคริปต์", "appsscript", "sheets"),
     "🟡 Lean 490/เดือน = บอทแบบง่าย ไม่มี AI/ฐานข้อมูล/เซิร์ฟเวอร์จ๊ะ:\n"
     "• ตอบตามคีย์เวิร์ด (แก้คำตอบใน Google ชีท ไม่แตะโค้ด)\n"
    ...
    (WHY_490_KWS,
     ...
     "   490/เดือน เทียบกับค่าแรงขั้นต่ำวันละ 300+ บาท\n"
     "   บอทตอบลูกค้าแทน 24 ชม. — คุ้มกว่าเยอะ\n\n"
     "2️⃣ ถูกกว่าเจ้าตลาด ได้ครบกว่า\n"
     "   เจ้าอื่นเริ่ม 500฿/เดือน (ZWIZ/Chatme) — เรา 490 ถูกกว่า\n"
     "   ได้บอทตอบคีย์เวิร์ด + เก็บข้อมูลลง Google Sheets อัตโนมัติ\n\n"
     "3️⃣ ทำไมเป็นรายเดือน\n"
     "   Facebook/LINE เปลี่ยนหน้าตาบ่อย → ต้องมีคนอัปเดตดูแล\n"
    ...
    (("เฉพาะshopee", "ใช้เฉพาะ", "ใช้กับshopee", "ลาซาด้า", "lazada", "ติ๊กต็อก", "แพลตฟอร์มอื่น"),
     ...
     "🟢 Starter+ (990+) = ส่วนขายสินค้าผูกกับ Shopee Affiliate (ลิงก์/ค่าคอม/เช็คลิงก์ตายทำไว้ให้ Shopee)\n"
     "อยากใช้ Lazada/TikTok กับ Starter+ = งานพัฒนาเพิ่ม (ไม่ใช่แค่ตั้งค่า) จ๊ะ"),
    (("ไฟล์สินค้า", "ไฟล์csv", "csv", "คอลัมน์", "ลิงก์สินค้า", "ลิงก์ข้อเสนอ"),
     "📋 ไฟล์สินค้า = CSV (เปิดใน Excel/Google ชีท) มี 9 คอลัมน์ แต่จำเป็นแค่ 2 จ๊ะ:\n"
     "① ชื่อสินค้า → บอทใช้ค้นหา\n"
     "② ลิงก์ข้อเสนอ → ลิงก์ affiliate (ระบบตรวจก่อนโชว์)\n\n"
     "สลับลำดับ/ลบ/เพิ่มคอลัมน์ได้อิสระ (อ่านตามชื่อคอลัมน์ ไม่ใช่ตำแหน่ง) มีเทมเพลต sample-products.csv ให้แล้วจ๊ะ"),
    (("ฟีเจอร์เสริม", "addon", "add-on", "ฟีเจอร์เพิ่ม"),
     "➕ ฟีเจอร์เสริม (บวกเพิ่มจากแพ็กเกจ) จ๊ะ:\n"
     "• เปลี่ยนแบรนด์ (ชื่อ/เสียง/โลโก้) +2,500\n"
     "• แยกฐานข้อมูล +1,500\n"
     "• LINE OA เพิ่ม +1,000\n"
     "• เพจ Facebook เพิ่ม +500\n"
     "• สร้างคอนเทนต์ AI +500\n"
     "• Support เร่งด่วน +500\n\n"
     "ฟีเจอร์ที่รวมในแพ็กเกจแล้วไม่คิดซ้ำนะจ๊ะ"),
    (("ค่าบริการไลน์", "ไลน์แพ็กเกจ"),
    ...
    (("เชื่อถือ", "น่าเชื่อ", "ไว้ใจ", "โกง", "มิจฉาชีพ", "หลอกลวง", "เชื่อได้ไหม", "โดนหลอก"),
     ...
     "• ราคาสุดท้ายดูในแอป Shopee ป้าไม่บวกเพิ่มและไม่เก็บเงินเอง\n"
     "• เก็บข้อมูลเฉพาะชื่อ + ข้อความ (90 วัน) ตาม PDPA ลบได้ทุกเมื่อ\n"
     "• ของโชว์ = ลิงก์ที่ตรวจแล้วเท่านั้น (ลิงก์ตาย/ปลอมไม่ขึ้น)\n\n"
     "ลองพิมพ์ชื่อสินค้าที่อยากได้ แล้วดูรีวิว/ยอดขายจริงได้เลยจ๊ะ 😊"),
    (("บวกเพิ่ม", "บวกเงิน", "บวกราคา", "เท่ากับแอป", "ราคาเดียวกัน", "ขึ้นราคา", "คิดเงิน"),
     "💰 ป้าเข็มไม่ตั้งราคาสินค้าและไม่บวกเพิ่มจ๊ะ — ราคาสุดท้ายยึดตามที่ Shopee แสดงหลังเลือกสินค้า/คูปอง\n"
     "ป้าเข็มเป็นนายหน้า — ได้ค่าคอมจาก Shopee/แบรนด์ ไม่เก็บเพิ่มจากลูกค้า\n"
     "กดซื้อไปที่แอป Shopee จ่ายตามราคาร้านค้าโดยตรงเลยจ๊ะ"),
    (BUILD_TIME_KWS,
     "⏱️ ระยะเวลาทำบอทให้แม่ค้า จ๊ะ:\n"
     "• 🟡 Lean 490 — 1 วัน (ตั้งค่าไฟล์เดียว + ส่งคู่มือ)\n"
     "• 🟢 Starter 990 — 2-3 วัน (สินค้า + คีย์ AI + เช็กลิงก์)\n"
     "• 🔵 Business 1,990 — 5 วัน (+ โพสต์ FB + เรดาร์ + แดชบอร์ด)\n"
     "• 🟣 White-Label 4,990 — 1-2 สัปดาห์ (เปลี่ยนแบรนด์ + ฐานข้อมูลแยก)\n"
    ...
    (("มาตรฐานการบริการ", "มาตรฐานบริการ", "บริการลูกค้า", "ประสบการณ์ลูกค้า",
     ...
     "1️⃣ การต้อนรับที่อบอุ่น\n"
     "   • ทักทายด้วยรอยยิ้ม\n"
     "   • เป็นกันเองและสุภาพ\n"
     "   • พร้อมให้บริการเสมอ\n\n"
     "2️⃣ การรับฟังอย่างตั้งใจ\n"
     "   • เข้าใจความต้องการของคุณ\n"
     "   • ไม่ขัดจังหวะขณะพูด\n"
     "   • ยืนยันข้อมูลเพื่อความถูกต้อง\n\n"
     "3️⃣ การนำเสนอทางเลือกที่เหมาะสม\n"
     "   • แนะนำสินค้า/บริการที่ตรงจุด\n"
     "   • ให้ข้อมูลที่ชัดเจนและครบถ้วน\n"
     "   • ตอบข้อสงสัยอย่างจริงใจ\n\n"
     "4️⃣ การดำเนินการที่รวดเร็วและมีประสิทธิภาพ\n"
     "   • ลดขั้นตอนที่ยุ่งยาก\n"
     "   • แจ้งสถานะการทำงาน\n"
     "   • แก้ไขปัญหาทันที\n\n"
     "5️⃣ การติดตามผลและขอบคุณ\n"
     "   • สอบถามความพึงพอใจ\n"
     "   • รับฟังความคิดเห็น\n"
...
def _github_button_card():
    """การ์ดแพ็กเกจ (รายเดือน 4 ระดับ + ขายขาด — เดิมเป็นการ์ดโค้ด GitHub)"""
    return package_flex_card()


# ข้อมูลแพ็กเกจ 5 ทางเลือก สำหรับการ์ด LINE Flex (สีแยกชัด: เหลือง/เขียว/น้ำเงิน/ม่วง/แดง)
# ฟีเจอร์ = สิ่งที่แม่ค้า "ได้" (benefit) ตามคำขายตลาดไทย — ไม่ใส่เทคนิค/โควตา/ค่าบริการ
# (เช่น ตอบ 24 ชม. / เก็บข้อมูลลูกค้า / แจ้งเตือน / Dashboard — อ้างอิง LINE OA 555฿, CherCode, ACRM 990฿)
PACKAGES = (
    {
        "key": "lean",
        "name": "🟡 Lean",
        "price": "490฿/เดือน",
        "color": "#F5A623",
        "tagline": "บอทตอบคีย์เวิร์ด + เก็บข้อมูลลูกค้า",
        "leadtime": "⏱️ เสร็จภายใน 1 วัน",
        "features": ("ตอบแชทอัตโนมัติ 24 ชม.", "เก็บข้อมูลลูกค้าลง Google Sheets",
                     "ต้อนรับอัตโนมัติ + รับฝากคำถาม", "แก้คำตอบเองได้ ไม่ต้องเขียนโค้ด"),
    },
    {
        "key": "starter",
        "name": "🟢 Starter",
        "price": "990฿/เดือน",
        "color": "#2ECC71",
        "tagline": "บอท AI เข้าใจภาษาไทย + ขายของ",
        "leadtime": "⏱️ เสร็จภายใน 2-3 วัน",
        "features": ("AI เข้าใจภาษาไทย ตอบเป็นธรรมชาติ", "ค้นหา/แนะนำสินค้า + ลิงก์ค่าคอม",
                     "จำความชอบลูกค้า แนะนำของตรงใจ", "เก็บประวัติแชท + สถิติคำถาม"),
    },
    {
        "key": "business",
        "name": "🔵 Business",
        "price": "1,990฿/เดือน",
        "color": "#3498DB",
        "tagline": "โพสต์อัตโนมัติ + หาคนซื้อให้",
        "leadtime": "⏱️ เสร็จภายใน 5 วัน",
        "features": ("โพสต์ Facebook อัตโนมัติ", "Radar หาคนอยากซื้อในกลุ่ม FB",
                     "แจ้งเตือนราคาตก + ของใหม่", "แดชบอร์ดรายงาน + ไม่จำกัดสินค้า/แชท"),
    },
    {
        "key": "whitelabel",
        "name": "🟣 White-Label",
        ...
        "leadtime": "⏱️ เสร็จภายใน 1-2 สัปดาห์",
        "features": ("เปลี่ยนชื่อ/เสียง/โลโก้เป็นแบรนด์คุณ", "ฐานข้อมูลแยก ลูกค้าเป็นของคุณ",
                     "หน้าเว็บ/โดเมนของคุณเอง", "support เร็วที่สุด + ตั้งค่าให้"),
    },
    {
        "key": "ขายขาด",
        "name": "🟠 ขายขาด",
        "price": "15,000–25,000฿",
        "color": "#E74C3C",
        "tagline": "ซื้อครั้งเดียว จบ",
        "leadtime": "⏱️ เสร็จภายใน 2-3 สัปดาห์",
        "features": ("จ่ายครั้งเดียว ไม่มีรายเดือน", "รวมติดตั้ง + ตั้งค่าให้ครบ",
                     "เหมาะร้านที่อยากจบทีเดียว"),
    },
)


def package_flex_card() -> FlexSendMessage:
    """การ์ด LINE Flex carousel 5 ใบ (Lean/Starter/Business/White-Label + ขายขาด)
    สีแยกชัด + ปุ่ม 'สนใจแพ็กเกจนี้' → ส่ง 'ยอด <แพ็กเกจ>' → ตอบหน้าชำระเงินของแพ็กเกจนั้น"""
    bubbles = []
    for p in PACKAGES:
        bubbles.append({
            "type": "bubble",
            ...
            "body": {
                ...
                "contents": [
                    ...
                    {"type": "text", "text": p["leadtime"], "size": "xs", "wrap": True,
                     "align": "center", "color": p["color"]},
                    {"type": "separator"},
                    {"type": "text", "text": "• " + "\n• ".join(p["features"]),
    ...
    alt_text = ("แพ็กเกจร้านป้าเข็ม 5 ทางเลือก: 🟡 Lean 490฿/เดือน · "
                "🟢 Starter 990฿/เดือน · 🔵 Business 1,990฿/เดือน · "
                "🟣 White-Label 4,990฿/เดือน · 🟠 ขายขาด 15,000–25,000฿")
...
def _mask_keyboard(t: str) -> str:
    """'คีย์บอร์ด'/'คีย์บอรด' → 'คียบอร์ด' และ 'คีย์เวิร์ด' → 'keyword'
    กัน keyword 'คีย์' (คีย์ AI) แมตช์กลางคำ 'คีย์บอร์ด'/'คีย์เวิร์ด' (Lean FAQ ตอบคีย์เวิร์ด)"""
    for kb in KEYBOARD_VARIANTS:
        t = t.replace(kb, "คียบอร์ด")
    t = t.replace("คีย์เวิร์ด", "keyword")
    return t
...
def is_bot_payment_request(text: str) -> bool:
    """ลูกค้าถามวิธีจ่ายเงินค่าบอท? (ปุ่มลัด '💰 วิธีจ่ายเงิน' ส่ง 'วิธีจ่ายค่าบอท')"""
    t = _mask_keyboard((text or "").strip().lower().replace(" ", ""))
...
def handle_compare(db, raw_text: str, user, is_owner: bool = False):
    """เทียบ A กับ B — การ์ด 2 คอลัมน์โดยไม่อ้างราคาสินค้าตายตัว."""
...
def compare_flex_message(a, b, facts: list, spec_a: str = "", spec_b: str = "",
    ...
    """การ์ดเทียบ 2 คอลัมน์ใบเดียว — ชื่อ/สเปค/ยอดขาย + ปุ่มดูราคาสด
    ...
    def col(p, tag, spec):
        contents = [
            ...
            {"type": "text", "text": sanitize_public_product_text(p.name), "size": "xs", "wrap": True, "maxLines": 3, "weight": "bold"},
        ]
        if spec:
        ...
        contents += [
            {"type": "separator"},
            {"type": "text", "text": "🏷️ ดูราคาล่าสุดใน Shopee", "size": "xs",
             "weight": "bold", "color": "#E74C3C", "align": "center", "wrap": True},
        ...
        contents.append({"type": "button", "style": "primary", "color": "#E74C3C", "height": "sm",
                         "action": {"type": "uri", "label": "🛒 ดูราคาล่าสุด", "uri": p.affiliate_url}})
...
WHY_US_YOUTH = (
    ...
    "1️⃣ ราคาโปร่งใส\n"
    "ป้าเป็นนายหน้า ไม่บวกเพิ่ม — กดดูและชำระตามราคาล่าสุดใน Shopee โดยตรง\n\n"
    "2️⃣ คัดมาแล้วว่าดีจริง\n"
    "เฉพาะของที่ขายดีจริง + ลิงก์ไม่ตาย ถึงเข้าร้าน ไม่มีของกากมาให้เสียเวลา\n\n"
    "3️⃣ ข้อมูลจริง ไม่พาดหัวลวง\n"
    "ไม่โชว์ราคาตายตัวให้สับสน กดไปเห็นราคาตามตัวเลือกและโปรจริงที่หน้าร้าน\n\n"
...
WHY_US_ELDER = (
    ...
    "1️⃣ ราคาโปร่งใส\n"
    "ป้าเข็มเป็นนายหน้า ไม่บวกเพิ่ม — กดดูและชำระตามราคาล่าสุดบน Shopee โดยตรง\n\n"
    "2️⃣ คัดมาแล้วว่าดีจริง\n"
    "เฉพาะของที่ขายดีจริง + ลิงก์ตรวจแล้วไม่ตาย ถึงจะเข้าร้าน\n\n"
    "3️⃣ ข้อมูลจริง ไม่พาดหัวลวง\n"
    "ไม่โชว์ราคาตายตัวให้สับสน กดลิงก์ไปเห็นราคาตามตัวเลือกและโปรโมชันที่หน้าร้าน\n\n"
...
def why_us_text(tone: str = "neutral") -> str:
    ...
    return (
        ...
        "1️⃣ ราคาโปร่งใส\n"
        "ป้าเข็มเป็นนายหน้า ไม่ได้บวกเพิ่ม — กดดูและชำระตามราคาล่าสุดบน Shopee โดยตรง\n\n"
        "2️⃣ คัดมาแล้วว่าดีจริง\n"
        "เฉพาะของที่ขายดีจริง + ลิงก์ตรวจแล้วไม่ตาย ถึงจะเข้าร้าน"
        " ของยอดขายน้อย/ลิงก์เสีย ไม่มีโผล่ให้เสียเวลา\n\n"
        "3️⃣ ข้อมูลจริง ไม่พาดหัวลวง\n"
        "ไม่โชว์ราคาตายตัวให้สับสน ยอดขายเป็นข้อมูลประกอบ"
        " และกดลิงก์ไปเห็นราคาตามตัวเลือก/โปรโมชันที่หน้าร้าน\n\n"
        "4️⃣ ป้าเข็มจำได้ว่าคุณชอบอะไร\n"
...
# ปุ่ม "คุยกับป้าเข็ม" — แนะนำตัวบอท (ทำอะไรได้บ้าง) + เข้าเรื่องราคา/แพ็กเกจ
# (5 ขั้นตอนโชว์แล้วตอน "สวัสดี"/แอดเพื่อน ไม่ซ้ำที่นี่ — คนกดปุ่มนี้ส่วนใหญ่สนใจตัวบอท)
CHAT_BOT_PHRASES = ("คุยกับป้าเข็ม", "คุยกับแม่เข็ม")
CHAT_BOT_PROMPT = (
    "🤗 ป้าเข็มเป็นบอทไลน์ช่วยแม่ค้าขายของอัตโนมัติจ๊ะ:\n\n"
    "• ค้นหา/แนะนำสินค้า + ลิงก์ค่าคอม\n"
    "• ตอบแชทลูกค้า 24 ชม. + จำความชอบ (PDPA ปลอดภัย)\n"
    "• โพสต์ Facebook หาคนซื้อ (Business ขึ้นไป)\n"
    "• FAQ อัตโนมัติ + ฝากคำถามให้เจ้าของตอบทีหลัง\n\n"
    "💰 มี 5 แพ็กเกจ เริ่ม 490฿/เดือน — แตะปุ่ม \"ราคาบอท/แพ็กเกจ\" ด้านล่างได้เลยจ๊ะ 👇\n\n"
    "หรือพิมพ์ชื่อสินค้า/คำถามให้ป้าช่วยได้เลยนะคะ 😊"
)
...
STORE_QUESTION_SELF_SERVICE = (
    ...
    "• ราคาสุดท้ายดูในแอป Shopee หลังเลือกสินค้า/คูปอง ป้าเป็นนายหน้า ไม่บวกเพิ่ม\n"
...
def message_text(event):
    ...
    try:
        ...
        if leave_ts and not is_owner and normalized_text not in DELETE_PHRASES \
            ...
            # กด "คุยกับป้าเข็ม" → แนะนำตัวบอท + เข้าเรื่องราคา/แพ็กเกจ + รอรับคำถามถัดไป
            ...
            if len(extracted_nums) >= 10:
                ...
                # เข้าสู่โหมดเซลติกครอส 10 ใบ เริ่มต้น session รอรับตัวเลข
                TarotSessionManager.start_session(line_user_id, target_count=10)
                intro_msg = TextSendMessage(
                    text="🔮 [ศาสตร์เปิดดวงใหญ่ 10 ใบ — The Celtic Cross]\n"
                         "━━━━━━━━━━━━━━━━━━\n"
                         "🎴 ป้าเข็มล้างพลังงาน สับไพ่ 78 ใบ และกรีดคว่ำหน้าเรียงพัด 1 ถึง 78 ไว้ตรงหน้าท่านแล้วค่ะ\n\n"
                         "✨ เทคนิคการเลือกตามจิตสัมผัส:\n"
                         "ตัวเลข 1 ถึง 78 ไม่ใช่เลขตายตัว แต่คือ “ตำแหน่งไพ่บนโต๊ะทำนาย” จิตใต้สำนึกของท่านจะดึงดูดไพ่ที่ตรงกับชะตาชีวิตขึ้นมาเอง\n\n"
                         "👉 วิธีเลือกไพ่ 10 ใบ (พิมพ์ตามตัวอย่างนี้ได้เลยจ้า):\n"
                         "พิมพ์ตัวเลขตำแหน่งไพ่ 1 ถึง 78 มา 10 ตัว เว้นวรรค เช่น:\n"
                         "5 12 21 34 45 52 60 67 71 78\n\n"
                         "━━━━━━━━━━━━━━━━━━\n"
                         "🕯️ หรือจะค่อยๆ พิมพ์ส่งทีละเลข ป้าเข็มก็นับผังให้จนครบ 10 ใบเช่นกันค่ะ ✨"
                )
