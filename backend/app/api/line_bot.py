...
BOT_MANUAL_PHRASES = (
    ...
    "ค่าดูแล", "ค่ารายปี", "รายปี", "ค่าบำรุง",
...
# (ห้ามใช้ "จ่ายเงิน"/"โอนเงิน"/"ชำระเงิน" เดี่ยว — ชนคำถามสั่งซื้อสินค้าที่ตอบ "จ่ายที่ Shopee")
BOT_PAYMENT_KWS = (
    "จ่ายค่าบอท", "จ่ายมัดจำ", "จ่ายแพ็กเกจ", "วิธีจ่ายค่าบอท",
    "promptpay", "พร้อมเพย์", "บัตรเครดิต", "ตัดบัตร",
    "โอนค่าบอท", "ชำระค่าบอท", "จ่ายค่างวด", "ค่างวด",
...
def payment_reply_text() -> str:
    """ข้อความวิธีจ่ายเงินค่าบอท/มัดจำ — แยกค่าติดตั้ง 1,500 ออกจากค่าแพ็กเกจชัด ๆ พร้อมยอดมัดจำ/ส่งมอบ (ลูกค้าไม่ต้องคิดเอง ไม่มั่ว)"""
    pay_lines = []
    if OWNER_PROMPTPAY:
    ...
    return (
        ...
        "🔧 ค่าติดตั้งครั้งแรก (จ่ายครั้งเดียว): 1,500 บาท\n"
        "   ✨ White-Label ติดตั้งฟรี\n\n"
        "💰 แบ่งจ่าย 2 งวด ชัด ๆ จ๊ะ (มัดจำ 50% จ่ายก่อน · ที่เหลือจ่ายตอนส่งมอบ):\n"
        "🟡 Lean 490       → มัดจำ 245 · ส่งมอบ 1,745\n"
        "🟢 Starter 990    → มัดจำ 495 · ส่งมอบ 1,995\n"
        "🔵 Business 1,990 → มัดจำ 995 · ส่งมอบ 2,495\n"
        "🟣 White-Label 4,990 → มัดจำ 2,495 · ส่งมอบ 2,495\n"
        "🟠 ขายขาด 15,000–25,000 → มัดจำ 7,500–12,500 · ส่งมอบ 7,500–12,500\n\n"
        "💡 ยอดส่งมอบ = ครึ่งหลังของแพ็กเกจ + ค่าติดตั้ง 1,500\n"
        "   (White-Label ติดตั้งฟรี · ขายขาดไม่มีค่าติดตั้ง)\n"
        "   หลังส่งมอบ จ่ายรายเดือนต่อตามแพ็กเกจที่เลือก\n\n"
        + "\n".join(pay_lines)
...
PACKAGE_PAYMENTS = (
    {"key": "lean", "label": "🟡 Lean", "name": "🟡 Lean 490",
     "deposit": "245 บาท", "delivery": "1,745 บาท (ครึ่งหลัง 245 + ค่าติดตั้ง 1,500)",
     "total": "1,990 บาท", "monthly": "490 บาท/เดือน"},
    {"key": "starter", "label": "🟢 Starter", "name": "🟢 Starter 990",
     "deposit": "495 บาท", "delivery": "1,995 บาท (ครึ่งหลัง 495 + ค่าติดตั้ง 1,500)",
     "total": "2,490 บาท", "monthly": "990 บาท/เดือน"},
    {"key": "business", "label": "🔵 Business", "name": "🔵 Business 1,990",
     "deposit": "995 บาท", "delivery": "2,495 บาท (ครึ่งหลัง 995 + ค่าติดตั้ง 1,500)",
     "total": "3,490 บาท", "monthly": "1,990 บาท/เดือน"},
    {"key": "whitelabel", "label": "🟣 White-Label", "name": "🟣 White-Label 4,990",
     "deposit": "2,495 บาท", "delivery": "2,495 บาท (ติดตั้งฟรี)",
     "total": "4,990 บาท", "monthly": "4,990 บาท/เดือน"},
    {"key": "ขายขาด", "label": "🟠 ขายขาด", "name": "🟠 ขายขาด 15,000–25,000",
     "deposit": "7,500–12,500 บาท", "delivery": "7,500–12,500 บาท (เท่ากับมัดจำ)",
     "total": "15,000–25,000 บาท", "monthly": "ไม่มีรายเดือน (ซื้อขาดครั้งเดียว)"},
)
    ...
    for p in PACKAGE_PAYMENTS:
        if p["key"] == key:
            return (
                ...
                f"• รวมจ่ายครั้งแรก: {p['total']}\n"
                f"• หลังส่งมอบ: {p['monthly']}\n\n"
...
BOT_MANUAL_SECTIONS = [
    ...
    # --- Lean Stack / แพลตฟอร์ม / ไฟล์สินค้า / ฟีเจอร์เสริม / ค่าดูแล / ค่า LINE OA (ขายบอท) ---
    ...
    (WHY_490_KWS,
     ...
     "   ถาม \"ขายขาด\" → จ่ายครั้งเดียว 15,000–25,000 + ค่าดูแลรายปี\n\n"
    ...
    (("ค่าดูแล", "ค่ารายปี", "รายปี", "ค่าบำรุง"),
     "🔧 ค่าดูแล/อัปเดตรายปี (M/A) ตามแพ็กเกจจ๊ะ:\n"
     "🟡 Lean 1,500 · 🟢 Starter 2,500 · 🔵 Business 3,500 · 🟣 White-Label 5,000 บาท/ปี\n"
     "(ครอบค่าซ่อมเมื่อ Facebook/Shopee เปลี่ยนหน้าเว็บ) จ่ายหลังปีแรกจ๊ะ"),
    (("ค่าบริการไลน์", "ไลน์แพ็กเกจ"),
    ...
    (("เชื่อถือ", "น่าเชื่อ", "ไว้ใจ", "โกง", "มิจฉาชีพ", "หลอกลวง", "เชื่อได้ไหม", "โดนหลอก"),
     ...
     "• ราคาเท่ากับในแอปเป๊ะ ไม่บวกเพิ่ม ไม่เก็บเงินเอง\n"
    ...
    (BUILD_TIME_KWS,
     ...
     "⏳ เริ่มนับวันเมื่อยืนยันสั่งทำ + จ่ายมัดจำ 50% ของแพ็กเกจครบแล้วนะจ๊ะ "
    ...
    (PERPETUAL_KWS,
     ...
     "• ค่าดูแล/อัปเดตรายปี (M/A) 2,500–5,000/ปี หลังปีแรก — ครอบค่าซ่อมเมื่อ Facebook/LINE เปลี่ยน UI\n"
...
PACKAGES = (
    ...
    },
    {
        ...
        "features": ("จ่ายครั้งเดียว ไม่มีรายเดือน", "รวมติดตั้ง + ตั้งค่าให้ครบ",
                     "M/A หลังปีแรก = ค่าซ่อม/อัปเดตเมื่อ FB/LINE เปลี่ยน", "เหมาะร้านที่อยากจบทีเดียว"),
    },
)
