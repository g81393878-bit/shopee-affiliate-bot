...
BOT_MANUAL_SECTIONS = [
    ...
    (("สมัครไลน์oa", "สมัครlineoa", "สมัครไลน์", "สมัครบัญชีไลน์", "เปิดไลน์oa", "lineoaสมัคร"),
     "💬 สมัคร LINE OA (ฟรี ~5 นาที) จ๊ะ — เข้า manager.line.biz → เข้าสู่ระบบด้วย LINE ที่มีอยู่ → "
     "สร้างบัญชีร้านค้า → ได้หน้าร้าน + QR ให้ลูกค้าแอด จากนั้นเข้า LINE Developers → Messaging API "
     "→ จด Channel Access Token + Channel Secret ไปใส่ใน .env แล้ว deploy (ขั้นละเอียดอยู่ใน docs/setup-guide.md ขั้น 1 จ๊ะ)"),
    ...
    (("supabaseคือ", "supabaseคืออะไร", "supabaseทำไม", "supabaseทําไม", "supabaseมีไว้ทำไม"),
     "🗄️ Supabase = ฐานข้อมูลคลาวด์ (สมุดบัญชีออนไลน์) จ๊ะ — ใช้จำสินค้า/ลูกค้า/ประวัติแชท ฟรี สำรองอัตโนมัติ "
     "ไม่หายถ้าเครื่องพัง และแยกจากเซิร์ฟเวอร์ (Render) เพื่อให้บอทเปิด 24 ชม. ข้อมูลก็ยังปลอดภัย "
     "สมัครที่ supabase.com ฟรี (ขั้นละเอียดอยู่ใน docs/setup-guide.md จ๊ะ)"),
