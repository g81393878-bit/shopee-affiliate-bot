...
- **แยก 2 flow กลุ่ม vs เพจ (status ต่างกัน อย่าให้ขัดกัน):** `ingest_facebook_leads` แยกตาม `lead.group_id` — **กลุ่ม** → `notification_status='pending_polling'` เข้าคิวให้บอท local poll `GET /api/admin/facebook-radar/tasks/pending` → แชร์ลงกลุ่ม → รายงาน `POST /tasks/{id}/status` → สำเร็จ `shared` / ล้ม `failed` (แจ้งเจ้าของ `radar_task_{completed|failed}_{task_id}`); **เพจป้าเข็ม** (ไม่ใช่กลุ่ม) → cooldown + daily limit + preflight → `pending` → `post_feed` → `posted`/`failed`. **`shared` ไม่นับใน `RADAR_MAX_DAILY_POSTS` และ category cooldown** (ทั้งคู่นับแค่ `posted/sent/pending`) → โควต้า/cooldown ของเพจกับกลุ่มแยกกันเด็ดขาด. Pivot แล้ว radar ไม่จับคู่สินค้าในคลัง (โพสต์ promo แทน — `matched_product_id=None`) → `check_category_cooldown_allowed` นับได้ทั้ง `Product.category` (event เก่า) และ `product_keyword` (event ใหม่) ผ่าน `or_`
...
- **`tools/fb_group_monitor_local.py` ต้องส่ง `--once` เสมอ** — ไม่งั้นวน loop (default interval 300s) ดูเหมือน hang. HTTP timeout = `DEFAULT_TIMEOUT_SECONDS=60` (อัปเดตจาก 15s แล้ว — พอเรียก production ที่ Groq+FB post ใช้เวลา >15s). **ห้ามใช้ `hash()` ทำ `fb_post_id`** — Python `hash()` ถูก salt ต่อ process (PYTHONHASHSEED) → id เปลี่ยนทุก run → dedup ข้าม run (`--state-file`) พัง → ส่ง lead เดิมซ้ำ → โพสต์ซ้ำ; ใช้ `hashlib.sha1(...)` deterministic
