...
3. **เสียงพากย์ป้าเข็มมาตรฐานเดียวกัน 100% (Voice Consistency & Natural Quality)**:
   - ล็อคเสียงพากย์หลักเป็น Microsoft Edge Neural TTS (`th-TH-PremwadeeNeural`, `rate="+20%"`, `volume=1.0`) คมชัด กระชับ ไวรัล สไตล์ Reels/Shorts ระดับสตูดิโอ ห้ามใช้ ffmpeg `atempo` เร่งสปีดคลื่นเสียงเด็ดขาด เพื่อป้องกันปัญหาเสียงพูดไวและเพี้ยน/แตกพร่า
...
6. **ผลิตคลิปเชื่อมโยง 8 หมวดหมู่เทรนด์ 100% (100% Trend-Driven Production)**:
...
7. **กฎ Hook 3 วินาทีหยุดนิ้วคนดู 100% (Strict 3-Second Viral Hook Requirement)**:
   - ทุกคอนเทนต์ (ทั้งไวรัล 90% และสินค้า 10%) ต้องขึ้นต้นด้วยประโยค **Hook 3 วินาทีแรก** เพื่อหยุดนิ้วคนดูบนฟีดทันที:
     - **บนการ์ดวิดีโอ (Visual Phase 1)**: ป้ายบนสุดและหัวข้อต้องโชว์ Hook 3 วิเด่นชัด
     - **เสียงพากย์ (Voiceover 0-3s)**: ประโยคเปิดต้องพูดประโยค Hook ที่ตื่นเต้น น่าติดตาม
     - **ชื่อคลิป & แคปชั่น (Title & Caption)**: บรรทัดแรกต้องขึ้นต้นด้วย Hook 3 วิเสมอ
   - ปลายทางแคปชั่น: คลิปสินค้าใส่ 2 ลิงก์ (Shopee + LINE OA) / คลิปไวรัลใส่ชวนคอมเมนต์ + ชวนกดติดตามช่อง
...
23. **โครงสร้างบัญชีและแพลตฟอร์มทั้งหมด 13 ช่องทาง (13-Channel Multi-Broadcast Master Matrix)**:
    - เอเจนต์ AI ทุกตัวต้องจดจำและยึดถือโครงสร้าง 13 ช่องทางนี้ตลอดเวลา ห้ามลืม ห้ามหลง ห้ามตัดทอนเด็ดขาด:
      1. 🔵 **Facebook Pages (3 เพจ)**: ยิงพร้อมกันทุก 30 นาที (เพจ 1: ป้าเข็ม ขายของ, เพจ 2: ป้าเข็ม ชี้เป้า, เพจ 3)
      2. 🔴 **YouTube Shorts (6 ช่องหมุนเวียน Round-Robin)**:
         - ช่อง 1: `@regency1229` (ป้าเข็ม ขายของ)
         - ช่อง 2: `@anda.review` (Anda Review)
         - ช่อง 3: `@healthgooddeals` (ของดีบอกต่อ)
         - ช่อง 4: `@anda.review99` (อันดา ป้ายยาของใช้ดี)
         - ช่อง 5: `@yibmareview-th` (หยิบมารีวิว)
         - ช่อง 6: `@paakhem-f7b` (ป้าเข็มบอกต่อ)
      3. ⚫ **TikTok Studio (4 ช่องหมุนเวียน Round-Robin ทุก 45 นาที ช่องละ 8 คลิป/วัน = 32 คลิป/วัน ห่างกัน 3 ชม./คลิป)**:
         - ช่อง 1: `@healthgooddeals` (`tiktok_cookies.json`)
         - ช่อง 2: `@cheepao.review` (`tiktok_cookies_2.json`)
         - ช่อง 3: `@pakhem.review99` (`tiktok_cookies_3.json`)
         - ช่อง 4: `@khonyangmefan` (`tiktok_cookies_4.json`)
      4. 📊 **สูตรการผลิต 90 / 10**: 90% คอนเทนต์ไวรัลหยุดดู 3 วิ (ข่าวด่วนโลก, ดารา, ทริคแก้ปัญหา, ดวง) / 10% สินค้า Shopee แท้ 100%
   24. **ระบบป้องกันคลิปและหัวข้อซ้ำ 100% (Strict 100% Anti-Duplicate Engine)**:
      - สมองกลตรวจจับ `is_topic_duplicate` ใน `standalone_content_generator.py` สกัดคำสำคัญ `extract_topic_keywords` และตรวจสอบความซ้ำซ้อน 3 ชั้น: Exact Match, Substring 18 ตัวอักษร, และ PyThaiNLP Keyword Overlap (≥60% หรือตรงกัน ≥3 คำสำคัญ)
      - บันทึกประวัติหลายชั้นถาวร: `tools/posted_content_history.json` (หัวข้อที่ผลิตทั้งหมด), `tools/posted_reels_title_history.json` (Facebook/YouTube), `tools/posted_tiktok_history.json` (TikTok แยกรายช่อง), `tools/posted_youtube_history.json` (YouTube Shorts)
      - สแกน RSS Stream สดใหม่ทุกข่าว และเลือกเฉพาะข่าวที่ยังไม่เคยผลิต ห้ามล็อค index อันดับ 1 เด็ดขาด
      - คลังทริคชีวิตและคนทำงาน 35+ หัวข้อ หมุนเวียน Index ด้วย `last_life_hack_index.txt` และ `last_work_tip_index.txt`
      - เคลียร์และคัดกรองคลิปซ้ำในคิวรอโพสต์ `pending_videos/` ด้วย `tools/sync_and_deduplicate.py`
   25. **กฎเหล็กการซิงค์ภาพ เสียงพากย์ และแคปชั่น 100% (Strict Visual-Voice-Caption Synchronization)**:
      - สินค้า Shopee (`auto_product_reels.py`) ล็อคโหมด `PRODUCT_HIGHLIGHT` เท่านั้น 100% ห้ามนำโหมดข่าว/ดวงมาครอบทับสินค้าเด็ดขาด เพื่อป้องกันปัญหาเสียงพากย์ข่าวด่วนแต่ภาพเป็นสินค้า
      - สคริปต์เสียงพากย์ในหมวดทริคชีวิตและคนทำงาน ต้องอ่านขั้นตอน `Step 1, Step 2, Step 3` ตรงกับการ์ดบนจอและแคปชั่น 100%
   26. **ระบบพื้นหลังสีและการ์ดข้อความอัตโนมัติ (Gradient Typography Fallback)**:
      - กรณีไม่มีภาพถ่ายจริง หรือภาพโหลดไม่สำเร็จ: ระบบจะไม่ยกเลิกการผลิตและไม่ปล่อยคลิปจอดำ แต่จะสร้าง **พื้นหลังสี Gradient สวยงามประจำหมวด** พร้อม **Hero Typography Card** และกล่องข้อความสรุปขั้นตอนตัวโตๆ คมชัด 100%
   27. **กฎเหล็กการแยกขาดระหว่างคอนเทนต์ไวรัลและสินค้า Shopee (Strict Pure Content vs Product Isolation & No Technical Stem Leaks)**:
      - **ห้ามนำ Hook สินค้า** (เช่น "ตัวนี้คนแย่งกันซื้อถล่มทลาย", "เตือนแล้วนะ! ใครยังไม่มีติดบ้าน") มาใส่ในคลิปข่าว/ดารา/ทริค/ดวง เด็ดขาด
      - **ห้ามนำป้ายพิกัด Shopee ว่างเปล่า** (`🛒 พิกัดสั่งซื้อของแท้ / โปรโมชั่น Shopee: 👉`) หรือลิงก์สินค้า มาใส่ในคลิปคอนเทนต์เพียว 100%
      - **ห้ามนำชื่อไฟล์ Technical หรือ Stem รหัสดิบ** เช่น `content_celebrity_trend_1788337430` หรือ `prod_...` ไปแสดงผลต่อผู้ใช้งานบนชื่อคลิป แคปชั่น หรือ Marquee Text เป็นอันขาด
      - ทุกโมดูล Uploader (YouTube Shorts `youtube_uploader.py`, TikTok Studio `system_runner.py`, Facebook Reels `uploader.py`) ต้องตรวจสอบ `is_pure_content` และ prefix ชื่อไฟล์อย่างเข้มงวดเสมอ
   28. **สถาปัตยกรรมแบ่งงานแบบแยกส่วน (Decoupled Split Architecture: VPS 24/7 Webhook vs Local Producer)**:
      - ☁️ **บน VPS (Always-On 24 ชม.)**: รันเฉพาะบริการ Webhook เพื่อตอบแชทลูกค้าตลอด 24 ชม. ประกอบด้วย `shopee-backend.service` (FastAPI Webhook), `cloudflared-tunnel.service` (Cloudflare Tunnel), และ `tunnel-watchdog.service` (Watchdog ซิงค์ URL อัตโนมัติทุก 15 วิ) **ปิดและ Disable บริการ `shopee-bot.service` บน VPS ถาวร** เพื่อไม่ให้กินทรัพยากร
      - 💻 **บนเครื่อง Local (On-Demand Workstation)**: รับผิดชอบงานโรงงานผลิตคลิป Reels/Shorts 9:16 Full HD (`auto_product_reels.py`), ตัวอัปโหลด Multi-Platform (`system_runner.py`, `uploader.py`, `tiktok_studio_uploader.py`), และ Telegram Commander
      - 🗄️ **Supabase PostgreSQL (Single Source of Truth)**: เชื่อมโยงข้อมูลสินค้า 2,472 รายการ และลูกค้า `users` เป็นศูนย์กลางเดียวกัน 100%
   29. **มาตรฐานศาสตร์ความเชื่อและโหราศาสตร์แท้ 100% (Strict Authentic Astrological & Esoteric Intelligence Standard)**:
       - **ห้ามทำเป็นเรื่องเล่นๆ ห้ามมโน ห้ามตลกขบขันเด็ดขาด**: ความเชื่อและโหราศาสตร์เป็นเรื่องลี้ลับ ละเอียดอ่อน และมีผู้คนศรัทธาจริง คอนเทนต์ในหมวด `LUCKY_FORTUNE` ต้องเปี่ยมด้วยความเคารพ สำรวม มีหลักวิชา และอ้างอิงศาสตร์แท้ 100%
       - **4 ศาสตร์หลักที่ต้องยึดถือ**:
         1. **คัมภีร์มหาทักษาปกรณ์ (Mahathaksa)**: การดูสีเสื้อมงคล 7 วัน ต้องตรงตามภูมิบริวาร, อายุ, เดช, ศรี, มูละ, อุตสาหะ, มนตรี, กาลกิณี ห้ามมโนสีมั่ว
         2. **ศาสตร์ราศีจักร & ดาวเคราะห์โคจร (Planetary Transits)**: อิงอิทธิพลของดาวประธานศุภเคราะห์ (ดาวพฤหัสบดี ๕) และดาวบาปเคราะห์ (ดาวเสาร์ ๗, พระราหู ๘) ต่อกลุ่มราศี 4 ธาตุ (ไฟ, ดิน, ลม, น้ำ)
         3. **ศาสตร์ตัวเลขและคู่ดาวมงคล (Star Pair Numerology)**: คู่ดาวแห่งความมั่งคั่ง เช่น ๒๔ (วาจาค้าขาย), ๓๖ (การเงินคล่องตัว), ๑๕ (ปัญญาบารมี), ๕๖ (มหาเศรษฐีศุภเคราะห์), ๗๘ (โชคใหญ่พลิกชีวิต)
         4. **ศาสตร์ฮวงจุ้ยเบญจธาตุ & ปราณชี่ (Five Elements Feng Shui)**: การจัดโต๊ะทำงาน (ซ้ายมังกรเขียว/ขวาเสือขาว), ฮวงจุ้ยกระเป๋าสตางค์, และลานหน้าบ้านหมิงถัง
       - **การสืบค้นร่องรอยดิจิทัลสด (Live Web Search Intelligence)**: ระบบต้องใช้ `fetch_authentic_astrology_insights()` ค้นหาข้อมูลเชิงลึกจาก Google/Tavily Search และสำนักโหรชั้นนำ สกัดแก่นความรู้ด้วย AI ก่อนนำมาผลิตคลิปจริง
    30. **ระบบบันทึกความเปลี่ยนแปลงและเอกสารโครงการลง Google Docs อัตโนมัติ (Automated Google Docs Changelog & Project Documentation Sync)**:
        - ทุกครั้งที่มีการแก้ไข ปรับปรุงโครงสร้าง เพิ่มฟีเจอร์ หรืออัปเดตระบบ เอเจนต์ AI **ต้องบันทึกสรุปการเปลี่ยนแปลง (Changelog Entry) ลงในเอกสาร Google Docs** ในโฟลเดอร์หลัก `Paa Khem Bot & Shopee Affiliate Master Workspace` (ID: `1jbNqUdiKf0Zdhk218QT_2yb4q63Vk2rj`) ผ่านคำสั่ง `python tools/google_docs_logger.py log --title "..." --details "..."` เสมอ
        - หากมีเอกสารสรุป แผนงาน หรือคู่มือใหม่ สามารถซิงค์ขึ้นเป็นเอกสาร Google Docs ตัวใหม่ได้ทันทีผ่านคำสั่ง `python tools/google_docs_logger.py sync --file <path> --title "..."` หรือสร้างเอกสารใหม่ด้วย `python tools/google_docs_logger.py create --title "..." --content "..."`
        - ยึดบัญชีหลัก **`regency2919@gmail.com`** เท่านั้น ห้ามแตะต้องบัญชีเทรด `regency1229@gmail.com` เด็ดขาด
    31. **กฎเหล็กแม่แบบมาตรฐานระดับสตูดิโอและขั้นตอนการทำงานของบอท (Strict Golden Master Template Compliance & 5-Step Bot SOP)**:
        - ปรัชญาหัวใจหลัก: *"กำหนดไว้ และทำตามอย่างเดียว"* (Zero Improvisation, Zero Guesswork, Zero Layout Drift)
        - เอเจนต์ AI และระบบผลิตวิดีโอทุกตัว **ต้องยึดถือและปฏิบัติตามแม่แบบมาตรฐานใน `reels_uploader/master_template_config.py`, `video_template_engine.py` และคู่มือฉบับเต็มใน `docs/manual/10_GOLDEN_MASTER_TEMPLATE_AND_BOT_SOP.md` อย่างเคร่งครัด 100% ห้ามคิดเอง ห้ามมโน ห้ามปรับเปลี่ยนพิกัด ขนาด หรือเลย์เอาต์เองเด็ดขาด**:
          1. **พิกัดและขนาด 5 Visual Zones (Canvas 1080x1920 9:16 Full HD)**:
             - **Zone 1: ป้ายหมวดหมู่บนสุด** (`Y=80..126`, `440x46px`, ขอบมน r=23, ขอบขาว 2px, ฟอนต์ Bold 23px)
             - **Zone 2: พาดหัว Hook 3 วิ** (`Y=140..310`, ล็อกไม่เกิน 45 ตัวอักษร, ตัดบรรทัดไม่เกิน 22 ตัว, ฟอนต์ Bold 38-48px พร้อมเงาดำ 4 ทิศทาง)
               - **กฎเหล็กข้อความบนสุด**: ต้องเป็นหัวข้อ/Hook ของเนื้อหานั้น **คงที่ตลอดทั้ง 3 เฟส (0-9s) 100%** (เฟส 1 สีเหลืองทอง `#FFEB3B`, เฟส 2-3 สีขาว `#FFFFFF`) **ห้ามเปลี่ยนเป็นคำเชิญชวนกดติดตาม หรือ CTA เด็ดขาด**
             - **Zone 3: กรอบภาพ Hero กลางจอ** (`Y=330..990`, `960x660px`, ขอบมน r=24, ขอบขาว 3px): ภาพถ่ายจริง 100% / หากไม่มีภาพ **ต้องสลับใช้ Hero Typography Card ด้วย Pillow ประจำโปรเจกต์ 100% (Rule 26)** ห้ามปล่อยกล่องดำ
             - **Zone 4: กล่องสาระสำคัญ 3 ข้อ** (`Y=1015..1635`, `960x620px`, 3 แถว แถวละ `888x136px`, เว้นวรรค 16px, r=18):
               - ไฮไลท์ทีละข้อแบบ Progressive (0-3s ข้อ 1, 3-6s ข้อ 2, 6-9s ข้อ 3)
               - **กฎเหล็กกำจัดคำขยะ 100%**: ทุกข้อต้องเป็นสาระสำคัญ/ข้อเท็จจริงจริง **ห้ามมีข้อความขยะ/Filler เช่น 'ติดตามช่องเพื่อรอเนื้อหาใหม่', 'กดติดตาม' หลุดเข้ามาใน 3 ข้อเด็ดขาด**
             - **Zone 5: แถบกระตุ้นท้ายคลิป** (`Y=1680..1775`, `960x95px`, พื้น Slate `#0F172A`, ขอบเขียว `#22C55E`, ข้อความเขียว `#4ADE80`): `• กดติดตามช่อง Anda • ทัก LINE: @137gsref •` ปลอดภัยจากปุ่มขวาและแถบล่าง TikTok/Shorts
          2. **ระบบตัดคำภาษาไทย**: ทุกข้อความต้องผ่าน `safe_thai_truncate()` พร้อมระยะผ่อนปรน `+4 ตัวอักษร` เสมอ เพื่อไม่ให้ตัดชื่อคนเฉพาะ (เช่น เร แม๊คโดแนลด์) หรือชื่อสถานที่ขาด
          3. **กฎ Zero Emoji บน Canvas**: ห้ามใส่ Unicode Emoji สีในข้อความบน Pillow Canvas เด็ดขาด ป้องกันการเกิดตัวอักษรเต้าหู้ `[]` ให้ใช้สัญลักษณ์มาตรฐาน `•` และ Vector Shapes เท่านั้น
          4. **เสียงพากย์ TTS มาตรฐานเดียว**: ล็อกความเร็ว `rate="+0%"` (สปีดปกติ ฟังชัดเจน ไม่รัว ไม่เพี้ยน) พร้อมหางเสียง `+0.4s` จบประโยคสมบูรณ์
          5. **ขั้นตอนการผลิต 5 สเต็ปสำหรับบอท (Bot 5-Step Pipeline SOP)**:
             - **สเต็ป 1**: ดึงและกรองข้อมูลดิบ (RSS ข่าวสด/คลังทริค + กรองความปลอดภัย 100% + เช็คหัวข้อซ้ำ `is_topic_duplicate`)
             - **สเต็ป 2**: สังเคราะห์และตรวจทานข้อความ (`title`, `hook_3s`, `step1`, `step2`, `step3` ไร้คำขยะ Filler + ผ่าน `format_master_*`)
...
- **SQLite dev schema drift**: `Base.metadata.create_all` (main.py) สร้างเฉพาะตารางที่ขาด ไม่ ALTER ตารางเดิม → `backend/affiliate_db.db` (dev SQLite) ดริฟท์จาก `app/models.py` ได้. อาการ: `/api/products/` หรือ `/api/products/{id}/analyze` 500 `no such column`. แก้เองด้วย `ALTER TABLE … ADD COLUMN` (เจอจริง: products ขาด `image_url`/`link_status`/`ai_score`/`price_checked_at`, contents ขาด `hook`/`problem`/`solution`/`cta`).
...
- **จดบันทึกข้อผิดพลาดและการแก้ไขทุกครั้ง (Strict Incident & Defect Logging):** ทุกครั้งที่ระบบมีข้อผิดพลาด เกิดบั๊ก หรือผลลัพธ์ไม่ตรงตามเป้าหมาย ต้องบันทึก: (1) ข้อผิดพลาดที่พบ (2) สาเหตุที่แท้จริง (Root Cause) (3) วิธีการแก้ไขเชิงโค้ด (4) ผลการทดสอบ ลงในคู่มือ `docs/manual/CASE_STUDY_*.md` และซิงค์เข้า **Master Google Doc (1dlKCrrrB3FvlAvHOwqvvI6ORnIcFAkKE7MB7TDxegaI)** และ **Changelog Doc (1DYJn3nce9DofaStc2q0g02J5xE9oNGSVODpXAv6GhDI)** เสมอ เพื่อให้ระบบพัฒนาต่อยอดได้อย่างยั่งยืนและไม่ทำผิดซ้ำ
- ไม่ทิ้งขยะ: ไฟล์ชั่วคราว `_*` (debug/จำลอง) ลบก่อน commit และรักษาความสะอาดของโค้ด ไม่สร้างสคริปต์ซ้ำซ้อนให้เปลืองโทเคน
