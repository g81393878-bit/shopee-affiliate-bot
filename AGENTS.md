...
- Env vars marked `sync: false` in `render.yaml` (DATABASE_URL, LINE_CHANNEL_ACCESS_TOKEN, LINE_CHANNEL_SECRET, GEMINI_API_KEY, GROQ_API_KEY) are set manually in the Render dashboard; `LLM_PROVIDER=groq` is baked in. The Render CLI has no env-var subcommand — env vars can only be set in the dashboard (or Management API). Groq is OpenAI-compatible (`base_url=https://api.groq.com/openai/v1`), default model `llama-3.3-70b-versatile` (override with `GROQ_MODEL`); provider values are `gemini` | `openai` | `groq`. Render free tier spins down after ~15 min of inactivity — `/health` exists solely to be pinged by cron-job.org every 10 min to keep the bot warm.
