...
- **`7477409` fix(cron): กรอง cooldown หมวดใน SQL ก่อน limit 20** — เดิมดึง 20 ตัวแรก (คอมสูง) แล้วกรองหมวดที่เพิ่งโพสต์ใน Python → ถ้าติด cooldown หมด ระบบเงียบทั้งที่มีสินค้าพร้อม 1,000+ ตัว (เจอจริง 18/08) — ยังไม่ push/deploy
- **feat(fb): ป้ายกำกับโพสต์ + คิวแชร์ลงกลุ่ม** (commit ถัดจาก `7477409`) — ป้าย `🛍️ โพสต์ขายสินค้า` / `👵 โพสต์แนะนำบอท` · ตารางใหม่ `group_share_tasks` + hook เข้าคิวทุกโพสต์สำเร็จ · API `/api/admin/group-shares/{pending,{id}/status,}` · `bot/run_campaign.py share --from-queue` — เทสต์รวม 1154 passed — ยังไม่ push/deploy
...
- **Deploy ปัจจุบัน (production):** commit `3f9ddad` live (deploy `dep-da1snbmi8c9s73akrqk0`) — มีฟีเจอร์สลิปครบ (รับรูป → OCR qwen → เทียบยอด → แจ้งเจ้าของ Flex → หน้าแอดมิน → `/สลิป`)
- **`FB_CONTENT_POST_INTERVAL` บน Render ยังไม่ตั้ง** — โพสต์แนะนำบอท/ข่าว/ร้านปิดอยู่ (ตั้ง 1440 = วันละ 1 ครั้ง ถึงเปิด) · `FB_AUTO_POST_INTERVAL=60` = โพสต์สินค้า 1 ตัว/ชม. ทำงานอยู่
