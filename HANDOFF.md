...
- **เทสต์แชร์กลุ่มจริง 18/08: วงจรครบ + แชร์สำเร็จ 1/3 กลุ่ม** — bot local poll → claim → แชร์จริงลงกลุ่ม 'ตลาดขายของออนไลน์...' (`[OK] dialog ปิด`) → รายงานกลับ → DB `status=shared` + `shared_at` ✓ · กลุ่มอีก 2 (ซื้อ-ขายออนไลน์/แวะมาป้ายยา) ล้มเพราะ **บัญชี FB ไม่ได้เป็นสมาชิกกลุ่ม** (FB คืนเฉพาะกลุ่มที่เป็นสมาชิก) — ต้องให้เจ้าของกด join กลุ่มก่อน · `bot/share_group.py` แก้ให้ทน UI แปรปรวนของ FB: retry กดแชร์/กลุ่ม + locator `normalize-space(.)` (span ซ้อน) + ยืนยันเลือกกลุ่มด้วยปุ่มโพสต์ (การแชร์โพสต์ไม่มีช่องแคปชั่น) — สคริปต์ local ไม่ต้อง deploy
- **เวลาในตาราง `bot_purchases`:** `created_at` (รับเรื่อง) · `paid_at` (โอน) · `confirmed_at` (เริ่มทำ) — แสดงเป็นเวลาไทย UTC+7
- **แชร์กลุ่มอัตโนมัติ (local):** Task Scheduler `PaKhem Share Morning/Noon/Evening` 08:00/12:00/18:00 → `bot/run_share_auto.bat` → แชร์โพสต์คิวล่าสุด 1 ตัว/รอบ (โควต้า 3/วัน) ลง 20 กลุ่ม — เครื่องต้องเปิด + ล็อกอิน Windows อยู่ (Chrome ต้องโชว์บนจอ); ตัวนับ `fb_daily_share.json` (gitignored); ลบงาน: `schtasks /delete /tn "PaKhem Share Morning"` (Noon/Evening เช่นกัน)