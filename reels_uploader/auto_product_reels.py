...
def build_voice_script(product_name: str, price: float, category: str, seed_id: int = 0) -> str:
    ...
    # 3. เจาะ Pain Point ราย 8 หมวดหมู่ (ไม่มีคำทักทายเวิ่นเว้อ จี้ปัญหาใน 3 วินาทีแรกทันที)
    if any(k in full_text for k in ["สัตว์", "หมา", "แมว", "อึ", "ฉี่", "ขน", "แผ่นรอง", "pet"]):
        hooks = [
    ...
    elif any(k in full_text for k in ["รีโมท", "สมาร์ต", "โคมไฟ", "พัดลม", "โซล่า", "หลอดไฟ", "smart", "wifi", "tuya"]):
        hooks = [
            f"เบื่อไหมที่ต้องคอยเดินหารีโมท แอร์ ทีวี พัดลม เป็นสิบๆ อัน? {short_title} คุมผ่านมือถือได้ทั้งบ้าน สั่งซื้อที่ลิงก์ในแคปชั่นนะลูก",
            f"เปลี่ยนบ้านธรรมดาให้เป็นสมาร์ตโฮมง่ายๆ! {short_title} สั่งงานไว ของแท้ทนทาน กดสั่งซื้อที่ลิงก์ในแคปชั่นได้เลยจ้า",
            f"เปิดปิดเครื่องใช้ไฟฟ้าได้จากทุกที่ทั่วโลก! {short_title} ติดตั้งง่าย ไม่ยุ่งยาก ช้อปของแท้ที่ลิงก์ในแคปชั่นนะลูก"
        ]
    elif any(k in full_text for k in ["ผ้า", "เช็ด", "ไมโคร", "คราบ", "จัดระเบียบ", "ตู้", "ทิชชู่", "ทำความสะอาด"]):
        hooks = [
    ...
    elif any(k in full_text for k in ["ครัว", "กระทะ", "หม้อ", "แก้ว", "อาหาร", "สาหร่าย", "ตะหลิว", "มีด", "จาน"]):
        hooks = [
            f"ทำอาหารแล้วกระทะติด ล้างยากจนเหนื่อย? {short_title} ทนความร้อนสูง ล้างง่ายมาก สั่งซื้อที่ลิงก์ในแคปชั่นนะลูก",
            f"สายทำอาหาร สายของกินต้องมีติดครัวไว้! {short_title} อร่อย สะอาด เกรดพรีเมียม กดสั่งซื้อที่ลิงก์ในแคปชั่นได้เลยจ้า",
            f"ไอเทมลับประจำห้องครัวที่แม่บ้านยกนิ้วให้! {short_title} ปลอดภัย ได้มาตรฐาน สั่งซื้อที่ลิงก์ในแคปชั่นได้เลยนะลูก"
        ]
    elif any(k in full_text for k in ["หูฟัง", "ฟิล์ม", "กล้อง", "บลูทูธ", "ไอที", "แกดเจ็ต", "เคส", "สายชาร์จ"]):
        hooks = [
    ...
    elif any(k in full_text for k in ["ปวด", "เมื่อย", "นวด", "สุขภาพ", "หลัง", "คอ", "ไหล่", "เบาะ", "ยา"]):
        hooks = [
    ...
    elif any(k in full_text for k in ["สิว", "หน้า", "สบู่", "ผิว", "ครีม", "เซรั่ม", "บำรุง", "ความงาม", "สำลี"]):
        hooks = [
    ...
    elif any(k in full_text for k in ["รถ", "แดด", "ช่าง", "กีฬา", "เดินทาง", "ร่ม", "ปั๊ม"]):
        hooks = [
        ...
        hooks = [
            f"มีปัญหาหาของดีราคาโดนใจไม่เจอใช่ไหม? {short_title} ตัวนี้ยอดขายปัง รีวิวแน่น สั่งซื้อที่ลิงก์ในแคปชั่นได้เลยนะลูก",
            f"ของดีคุณภาพพรีเมียมที่ต้องมีติดบ้าน! {short_title} การันตีของแท้ร้อยเปอร์เซ็นต์ กดสั่งซื้อที่ลิงก์ในแคปชั่นได้เลยจ้า",
            f"ใครอยากได้ไอเทมเด็ดคุ้มค่าเงินทุกบาท แนะนำ {short_title} ใช้งานดีมาก ช้อปที่ลิงก์ในแคปชั่นนะลูก"
        ]
